...
//	}
     ...
     */
    public static String newFileName( String randomName,String fileName) {
        if (isNotEmpty(fileName)) {
            int i = fileName.lastIndexOf(".");
            String suffixName = fileName.substring(i);
            if(".gif".equals(suffixName) || ".jpg".equals(suffixName) || ".jpeg".equals(suffixName)){
     ...
     * @Author: maoLinLong
     * @Date: 2018/7/23
     * @Description: 截取图片储存路径前缀
     * @Param: [str]
     * @return: java.lang.String
     */
    public static String cutPrefix(String str) {

        int fast = str.lastIndexOf("/");
        return str.substring(fast);
    }
     ...
     * @return: boolean
     */
    public static boolean inList(Object item,List<Object> items){
        if(isNotEmpty(item) && isNotEmpty(item)){
    ...
    /**
     *Unix时间(精确到秒的) 转换成 日期格式的字符串
     * @param uinxTime
     * @param formats
     * @return
     */
    public static String unixToDate(Long uinxTime,String formats){
        formats = isEmpty(formats) ? "yyyy-MM-dd HH:mm:ss" :formats;
        SimpleDateFormat sdf = new SimpleDateFormat(formats);
        String date = new SimpleDateFormat(formats, Locale.CHINA).format(new Date(uinxTime * 1000));
        return date;
    }
